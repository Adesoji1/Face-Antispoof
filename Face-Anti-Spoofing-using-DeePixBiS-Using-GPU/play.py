...
class Trainer():
    def __init__(self, train_dl, val_dl, model, epochs, opt, loss_fn):
        self.device = get_default_device()
        self.train_dl = DeviceDataLoader(train_dl, self.device)
        self.val_dl = DeviceDataLoader(train_dl, self.device)
        self.model = to_device(model, self.device)
        # self.model = model
        self.epochs = epochs
        self.opt = opt
        self.loss_fn = loss_fn
        print(self.train_dl.device)
    ...
    def train_one_epoch(self, num):
        ...
        for batch, (img, mask, label) in enumerate(self.train_dl):
            img, mask, label = img, mask, label #img.to(self.device), mask.to(self.device), label.to(self.device)
